...
async def mock_get_current_user():
    return User(chat_bot_day_requests=2)  # Assuming the user has made 2 requests
...
async def test_check_daily_limit_user_within_limit(monkeypatch):
    ...
    monkeypatch.setattr("your_module.get_current_user", AsyncMock(return_value=User(chat_bot_day_requests=2)))
...
async def test_check_daily_limit_user_exceeds_limit(monkeypatch):
    ...
    monkeypatch.setattr("your_module.get_current_user", AsyncMock(return_value=User(chat_bot_day_requests=5)))
