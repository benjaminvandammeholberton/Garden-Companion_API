...
class VegetableInfoCreate(BaseModel):
    ...
    start_indoor: conint(ge=1, le=52) = Field(..., title='Week number to start indoor sowing')
    start_outdoor: conint(ge=1, le=52) = Field(..., title='Week number to start outdoor sowing')
    end: conint(ge=1, le=52) = Field(..., title='Last week number for sowing')
    water_needs: conint(ge=0, le=5) = Field(..., title='Water Needs (scale: 0 to 5)')
    cold_resistance: conint(ge=0, le=3) = Field(..., title='Cold Resistance (scale: 0 to 3)')
...
class VegetableInfoUpdate(BaseModel):
    ...
    cold_resistance: Optional[conint(ge=0, le=3)] = Field(None, title='Cold Resistance (scale: 0 to 3)')
